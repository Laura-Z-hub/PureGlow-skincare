...
function requireLogin(): array
{
    ...
    if (empty($_SESSION['user'])) {
        ...
        if (str_contains($requestUri, '/api/')) {
            sendJson(['error' => 'Please sign in first'], 401);
        }

        if (str_contains($requestUri, '/admin/')) {
            header('Location: /skincare/admin/login.php');
            exit;
        }

        header('Location: /skincare/signin.php');
        exit;
    }
...
function requireCustomer(): array
{
    ...
    if (($user['role'] ?? '') === 'admin') {
        ...
        if (str_contains($requestUri, '/api/')) {
            sendJson(['error' => 'Admin accounts cannot make purchases from the customer website.'], 403);
        }

        header('Location: /skincare/admin/dashboard.php');
        exit;
    }
